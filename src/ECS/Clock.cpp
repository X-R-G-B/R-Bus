...
std::size_t Clock::create(bool deferStart)
{
    if (deferStart) {
        ...
        _clocks.emplace_back(std::chrono::system_clock::now());
    }
...
std::size_t Clock::elapsedMillisecondsSince(std::size_t id)
{
    ...
    auto val = std::chrono::system_clock::now() - _clocks[id].value();
...
{
    _clocks[id] = std::chrono::system_clock::now();
}

void Clock::restart(std::size_t id, std::time_t time)
{
    _clocks[id] = std::chrono::system_clock::from_time_t(time);
}
