...
namespace Systems::ParallaxSystems {
    ...
    static void initParallaxEntity(
        nlohmann::basic_json<> &elem,
        const float maxOffsideParallax = 0)
    {
        std::size_t id        = Registry::getInstance().addEntity();
        Raylib::Sprite parralax = {Json::getInstance().getDataFromJson(elem, "spritePath"), Json::getInstance().getDataFromJson(elem, "width"), Json::getInstance().getDataFromJson(elem, "height"), id};
    ...
    {
        std::vector<nlohmann::basic_json<>> parallaxData = Json::getInstance().getDataByJsonType("parallax", JsonType::DEFAULT_PARALLAX);

        for (auto &elem : parallaxData) {
            initParallaxEntity(elem);
            if (Json::getInstance().getDataFromJson(elem, "copy") != nullptr && Json::getInstance().getDataFromJson(elem, "copy") == true) {
